...
export class CodeViewer {
  ...
  protected initializeTabsAndIts_Content(): this {
    ...
    for (const [ tabContentNumber, tabContent ] of this.tabsContent.entries()) {
      ...
      const tabContentData: CodeViewer.TabData = currentTabDataProcessingResult.processedData;
      ...
      if (tabContentData.isActive && !hasAtLeastOneTabContentBeenSetToActive) {
      ...
      getExpectedToBeSingleElement({
        ...
        context: currentTab
      }).textContent = tabContentData.language;


      if (isNotUndefined(tabContentData.fileLabel)) {
        getExpectedToBeSingleElement({ selector: ".CodeViewer-Tab-FileLabel", context: currentTab }).
            textContent = tabContentData.fileLabel;
      } else {
        getExpectedToBeSingleElement({ selector: ".CodeViewer-Tab-FileLabel", context: currentTab }).remove();
      }

      this.tabs.push(currentTab);
      this.initializePartialCodeListingsIfTheyArePresent({ tabContent, targetLanguage: tabContentData.language });
    }
  ...
  protected initializePartialCodeListingsIfTheyArePresent(
    {
      tabContent,
      targetLanguage
    }: {
    ...
    const partialListings: NodeListOf<Element> = tabContent.querySelectorAll(".CodeViewer-PartialListing");
    ...
    partialListings.forEach((partialListing: Element): void => {
      // TODO 言語自身ではなく、此れの省略が要る。
      partialListing.dataset.language = targetLanguage;
    });
  }
